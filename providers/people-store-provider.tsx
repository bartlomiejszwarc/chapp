...
export const PeopleStoreProvider = ({children}: PeopleStoreProviderProps) => {
  const storeRef = useRef<PeopleStoreApi>();
  if (!storeRef.current) {
    storeRef.current = createPeopleStore();
  }

  return <PeopleStoreContext.Provider value={storeRef.current}>{children}</PeopleStoreContext.Provider>;
};

export const usePeopleStore = <T,>(selector: (store: PeopleStore) => T): T => {
  const peopleStoreContext = useContext(PeopleStoreContext);

  if (!peopleStoreContext) {
    throw new Error(`usePeopleStore must be used within PeopleStoreProvider`);
  }
